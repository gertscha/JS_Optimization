...
namespace JSOptimzer {
	...
	void Solution::parseFileAndInitSolution(std::ifstream& file) {
		...
		if (!(Utility::getNextInt(line, lineIndex, numTasks) && Utility::getNextInt(line, lineIndex, numMachines)
			&& Utility::getNextInt(line, lineIndex, completionT)))
		{
			ABORT_F("parsing error, first non-comment line needs to be three interges for 'task_cnt machine_cnt completetionTime'");
		}
		...
		// init outer vector
		solution = std::vector<std::vector<SolStep>>(m_machineCnt);
		// prepare problemRep, track sizes
		problemRep = std::vector<std::vector<SolStep*>>(numTasks);
		...
		while (std::getline(file, line)) {
			...
			solution[machineIndex] = std::vector<SolStep>(curSolStepCnt);
			...
			while (Utility::getNextInt(line, lineIndex, currentInt)) {
				...
				size_t solMachineSize = solution[machineIndex].size();
				...
				solution[machineIndex][stepIndex] = SolStep{ tid, tindex, machine, start, end };
				stepIndex++;
		...
		for (int i = 0; i < numTasks; ++i) {
			problemRep[i] = std::vector<SolStep*>(taskIndCnt[i] + 1, NULL);
		}
	}
	...
	bool Solution::saveToFile(const std::string& filepath, const std::string& filename) const
	{
		...
		if (file.is_open()) {
			...
			file << m_taskCnt << " " << m_machineCnt << " " << m_completionTime << std::endl;
			// output solution natrix
			for (unsigned int i = 0; i < m_machineCnt; ++i) {
				size_t len = solution[i].size();
				file << len << " - ";
	...
	bool Solution::validateSolution(const Problem& p) const
	{
		...
		const std::vector<std::vector<SolStep>>& sol = solution; // alias with shorter name
		size_t stepCnt = 0;
	...
	long Solution::getCompletetionTime()
	{
		...
		long res = 0;
		size_t mCnt = solution.size();
		for (unsigned int i = 0; i < mCnt; ++i) {
			long t = solution[i][solution[i].size() - 1].endTime;
			if (t > res)
				res = t;
		}
